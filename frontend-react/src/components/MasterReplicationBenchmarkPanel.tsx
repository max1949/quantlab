...
function dismissKey(projectId: string) {
  return `quantlab-replication-benchmark-dismissed-${projectId}`;
}
...
export default function MasterReplicationBenchmarkPanel({
  ...
  const matches = Boolean(benchmark && backtestDone && !validationDone && !dismissed);

  useEffect(() => {
    if (!matches || celebrated || benchmark?.oos_sharpe == null || yourBacktestSharpe == null) return;
    if (yourBacktestSharpe - benchmark.oos_sharpe >= 0.05) {
      burstConfetti(2400);
      setCelebrated(true);
    }
  }, [matches, celebrated, benchmark, yourBacktestSharpe]);

  if (!matches || !benchmark) return null;
  ...
  return (
    <div className="mb-4 card border border-amber-200 bg-gradient-to-r from-amber-50/90 to-orange-50/60 dark:border-amber-900 dark:from-amber-950/40 dark:to-orange-950/30">
      ...
      <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
        {verdict(benchmark, yourBacktestSharpe, {
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs">
        ...
        {yourBacktestSharpe != null && (
          <span className="rounded-full bg-white px-2.5 py-1 font-medium text-slate-800 ring-1 ring-amber-200 dark:bg-slate-900 dark:text-slate-100 dark:ring-amber-800">
            {d.yourBacktest(yourBacktestSharpe.toFixed(2))}
          </span>
        )}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <Link to={`/reports/${benchmark.report_id}`} className="btn text-xs" onClick={dismiss}>
          {masterLabel}
        </Link>
        <button
          type="button"
